...
function NameInput({ value, onChange, error }: { value: string; onChange: (v: string) => void; error: string }) {
  return (
    <div style={{ marginBottom: error ? '6px' : '12px' }}>
      <label style={{ display: 'block', fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '5px', textAlign: 'left', letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        Display Name
      </label>
      <input
...
function PasswordInput({ value, onChange, label, placeholder, optional }: {
  ...
  return (
    <div style={{ marginBottom: '12px' }}>
      <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '5px', letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        {label}
      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          maxLength={50}
          style={{
            width: '100%', padding: '9px 40px 9px 12px', fontSize: '14px',
  ...
  return (
    <button
      ...
      style={{
        ...
        border: 'none', borderRadius: '11px', cursor: 'pointer', fontFamily: 'inherit',
      }}
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <div style={{
            flex: 1, padding: '9px 12px', background: 'var(--bg-tertiary)',
            border: '1.5px solid var(--border)', borderRadius: '10px',
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ marginBottom: '12px' }}>
        ...
        <input
          ...
          maxLength={20}
          style={{
            width: '100%', padding: '9px 12px', fontSize: '15px', fontWeight: 700,
            letterSpacing: '0.18em', border: `1.5px solid ${roomError ? '#ef4444' : 'var(--border)'}`,
            borderRadius: '10px', background: 'var(--bg-primary)', color: 'var(--accent)',
...
export default function JoinScreen({ onJoin, joinError }: Props) {
  ...
  return (
    <div className="join-outer" style={{
      ...
      <style>{`
        @media (max-height: 700px), (max-width: 400px) {
          .join-outer { align-items: flex-start; padding: 12px; }
          .join-card { padding: 20px !important; border-radius: 16px !important; }
          .join-logo { width: 42px !important; height: 42px !important; border-radius: 12px !important; margin-bottom: 10px !important; }
          .join-logo svg { width: 20px !important; height: 20px !important; }
          .join-title { font-size: 18px !important; }
          .join-subtitle { font-size: 12px !important; margin-bottom: 14px !important; }
          .join-tabs { margin-bottom: 14px !important; }
        }
      `}</style>
    </div>
  )
